...
// 스위치를 누르고 떼면 -> FND에 숫자 출력 (0 ~ 1초) -> (00 ~ 99) : 10ms 단위
...
int pushed_time_value = 0, pushed_time_10ms = 0, pushed_time_100ms = 0;
...
int main(void)
{
    ...
    while (1)
    {
        ...
        bit_74HC595_shift16(numbers[pushed_time_10ms], numbers[pushed_time_100ms]);
    }
...
void SW_ISR1R(void)
{
    ...
    else
    {
        ...
        if (r_value >= 4096)
            r_value = 4095;
        pushed_time_value = r_value * 100 / 4095;
        if (pushed_time_value >= 100)
            pushed_time_value = 99;
        pushed_time_10ms = pushed_time_value % 10;
        pushed_time_100ms = pushed_time_value / 10;
        printf("R: %d, Pushed %dms\n", r_value, pushed_time_value * 10);
        r_pushed = 0;
    }
}
...
void SW_ISR2R(void)
{
    ...
    else
    {
        ...
        if (g_value >= 4096)
            g_value = 4095;
        pushed_time_value = g_value * 100 / 4095;
        if (pushed_time_value >= 100)
            pushed_time_value = 99;
        pushed_time_10ms = pushed_time_value % 10;
        pushed_time_100ms = pushed_time_value / 10;
        printf("G: %d, Pushed %dms\n", g_value, pushed_time_value * 10);
        g_pushed = 0;
    }
}
...
void SW_ISR3R(void)
{
    ...
    else
    {
        ...
        if (b_value >= 4096)
            b_value = 4095;
        pushed_time_value = b_value * 100 / 4095;
        if (pushed_time_value >= 100)
            pushed_time_value = 99;
        pushed_time_10ms = pushed_time_value % 10;
        pushed_time_100ms = pushed_time_value / 10;
        printf("B: %d, Pushed %dms\n", b_value, pushed_time_value * 10);
        b_pushed = 0;
    }
}
